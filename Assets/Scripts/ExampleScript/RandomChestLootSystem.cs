    ...
    void Start()
    {
        ...
        while (counter < amountOfChest)
        {
            ...
            while (creatingItemsForChest < itemAmountForChest)
            {
                ...
                if (true) //  Stratos edit: raffle <= inventoryItemList.itemList[randomItemNumber].rarity
                {
            ...
            {
                sI.storageItems.Add(inventoryItemList.getItemByID(itemsForChest[i].id));

                int randomValue = Random.Range(1, sI.storageItems[sI.storageItems.Count - 1].maxInStack);
                sI.storageItems[sI.storageItems.Count - 1].quantity = randomValue;
            }
