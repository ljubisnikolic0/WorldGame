...
public class DragItem : MonoBehaviour, IDragHandler, IPointerDownHandler, IEndDragHandler
{
    ...
    public void createDuplication(GameObject Item)
    {
        ...
        GameObject duplication = GameObject.FindGameObjectWithTag("MainInventory").GetComponent<Inventory>().addItemToInventory(item.id, item.quantity);
    ...
    public void OnEndDrag(PointerEventData data)
    {
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            if (newSlot != null)
            {
                ...
                if (sameItem)
                {
                    firstItemStack = firstItem.quantity < firstItem.maxInStack;
                    secondItemStack = secondItem.quantity < secondItem.maxInStack;
                }
                ...
                if (Inventory.GetComponent<Hotbar>() == null && Inventory.GetComponent<EquipmentSystem>() == null && Inventory.GetComponent<CraftSystem>() == null)
                {
                    ...
                    else
                    {
                        ...
                        if (newSlotChildCount != 0 && isOnSlot)
                        {
                            ...
                            if (sameItem)
                                fitsIntoStack = (firstItem.quantity + secondItem.quantity) <= firstItem.maxInStack;
                            ...
                            if (inventory.stackable && sameItem && firstItemStack && secondItemStack)
                            {
                                ...
                                if (fitsIntoStack && !sameItemRerferenced)
                                {
                                    ...
                                    if (secondItemGameObject.GetComponent<ConsumeItem>().duplication != null)
                                    {
                                        ...
                                        dup.GetComponent<ItemOnObject>().item.quantity = secondItem.quantity;
                                ...
                                else
                                {
                                    ...
                                    int rest = (firstItem.quantity + secondItem.quantity) % firstItem.maxInStack;

                                    //fill up the other stack and adds the rest to the other stack 
                                    if (!fitsIntoStack && rest > 0)
                                    {
                                        firstItem.quantity = firstItem.maxInStack;
                            ...
                            else
                            {
                                ...
                                int rest = 0;
                                if (sameItem)
                                    rest = (firstItem.quantity + secondItem.quantity) % firstItem.maxInStack;

                                //fill up the other stack and adds the rest to the other stack 
                                if (!fitsIntoStack && rest > 0)
                                {
                                    secondItem.quantity = firstItem.maxInStack;
                ...
                if (Inventory.GetComponent<EquipmentSystem>() != null)
                {
                    ...
                    if (newSlotChildCount != 0 && isOnSlot)
                    {
                        ...
                        if (sameItemType && !sameItemRerferenced) //
                        {
                            ...
                            {
                                if (false) //firstItem.itemType == ItemType.UFPS_Weapon
                                {
            ...
            {
                GameObject dropItem = new GameObject(); //Stratos - (GameObject)Instantiate(GetComponent<ItemOnObject>().item.itemDropModel)
