...
public class StorageInventory : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        while (creatingItemsForChest < randomItemAmount)
        {
            ...
            if (false) // Stratos edit: raffle <= inventoryItemList.itemList[randomItemNumber].rarity
            {
 		  int randomValue = Random.Range(1, inventoryItemList.itemList[randomItemNumber].getCopy().maxInStack);
    ...
    void Update()
    {
        ...
        if (distance <= distanceToOpenStorage && Input.GetKeyDown(InputManager.StorageCode))
        {
