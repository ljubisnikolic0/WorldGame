...
//        GetComponent<Status>().useMecanim = useMecanim;
        ...
        //str = GetComponent<Status>().attackDmg;
        //matk = GetComponent<Status>().wizardyDmg;
        ...
        if (!useMecanim)
        {
            //hurt = GetComponent<Animation>().CrossFade("Damage");
        }
    ...
    void Update()
    {
        ...
        //Magic
        if (Input.GetButtonDown("Fire2") && Time.time > nextFire && !isCasting && skillPrefab[skillEquip])
        {
    ...
    IEnumerator MagicSkill(int skillID)
    {
        ...
        if (skillAnimation[skillID])
        {
            ...
            //if (GetComponent<Status>().mana > manaCost[skillID])
            //{
    ...
    {
        if (sound.hurtVoice && GetComponent<Status>().getHealth >= 1)
        {
